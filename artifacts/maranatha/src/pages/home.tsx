...
import { useGetDiffusion } from "@workspace/api-client-react";
...
type Tab = "accueil" | "biblio" | "programme" | "dons";

export default function Home() {
  const { data: diffusion } = useGetDiffusion({
  ...
  const handleDonationWa = () => {
    if (diffusion?.n_whatsapp) {
      ...
      const message = encodeURIComponent(
        "Bonjour, voici la capture de mon don pour l'œuvre de Dieu."
      );
  ...
  const navItems: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "biblio", label: "Biblio", icon: <BookOpen className="w-4 h-4" /> },
    { id: "programme", label: "Programme", icon: <Calendar className="w-4 h-4" /> },
    { id: "dons", label: "Dons", icon: <HeartHandshake className="w-4 h-4" /> },
  ];
      ...
      <div className="w-full max-w-[450px] bg-background shadow-2xl relative flex flex-col">
        ...
        <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-md border-b border-border px-4 py-3 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h1 className="font-serif font-bold text-2xl tracking-tighter text-primary">
              MARANATHA
            </h1>
          ...
          <nav className="flex items-center justify-between text-xs font-semibold pb-1">
            {navItems.map((item) => (
              <button
                key={item.id}
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden pb-12">
          ...
          {/* ── ACCUEIL ── */}
          {activeTab === "accueil" && (
            <section className="p-4 pt-6 space-y-6">
              <Button
          ...
          {activeTab === "biblio" && (
            <section className="p-4 pt-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="font-sans font-bold text-xl uppercase tracking-wider text-foreground">
                  Bibliothèque
                </h2>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {["Prédications", "Louanges", "Témoignages", "Vidéos"].map((cat, i) => (
                  <div
                    key={i}
                    className="aspect-square bg-card border border-border rounded-xl flex flex-col items-center justify-center p-4 hover:border-primary/50 transition-colors cursor-pointer group shadow-sm"
                  >
                    {i === 3 ? (
                      <Youtube className="w-8 h-8 mb-2 text-red-500 group-hover:scale-110 transition-transform" />
                    ) : (
                      <BookOpen className="w-8 h-8 mb-2 text-primary/70 group-hover:scale-110 transition-transform" />
                    )}
            ...
            <section className="p-4 pt-6 space-y-4">
              ...
              <div className="grid gap-3">
                <div className="bg-card border border-border p-4 rounded-xl flex justify-between items-center shadow-sm">
                  <div>
                    <h3 className="font-bold text-primary">
                      {diffusion?.t_matin || "Culte Matinal"}
                    </h3>
                ...
                <div className="bg-card border border-border p-4 rounded-xl flex justify-between items-center shadow-sm">
                  <div>
                    <h3 className="font-bold text-primary">
                      {diffusion?.t_soir || "Culte du Soir"}
                    </h3>
            ...
            <section className="p-4 pt-6 space-y-4">
              ...
              <div className="space-y-3">
                {[
                  {
                    name: "Airtel Money",
                    num: diffusion?.n_airtel || "+243 000 000 000",
                    color: "bg-red-50 text-red-700 border-red-200",
                  },
                  {
                    name: "Orange Money",
                    num: diffusion?.n_orange || "+243 000 000 000",
                    color: "bg-orange-50 text-orange-700 border-orange-200",
                  },
                  {
                    name: "M-Pesa (Voda)",
                    num: diffusion?.n_voda || "+243 000 000 000",
                    color: "bg-blue-50 text-blue-700 border-blue-200",
                  },
                ].map((m, i) => (
                  <div
                    key={i}
                    className={`border p-4 rounded-xl flex justify-between items-center ${m.color}`}
                  >
